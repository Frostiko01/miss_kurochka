...
interface MenuItemCardProps {
  ...
  onAddToCart: (menuItemId: string, sizeId: string | null, spiceIds: string[]) => void
...
function SpiceModal({ item, initialSizeId, onAddToCart, onClose }: SpiceModalProps) {
  ...
  const spicesTotal = selectedSpiceIds.reduce((sum, id) => {
    const sp = spices.find((s) => s.id === id)
    return sum + (sp ? Number(sp.price) : 0)
  }, 0)
  const totalPrice = basePrice + spicesTotal

  const toggleSpice = (id: string) =>
    setSelectedSpiceIds((prev) =>
      prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]
    )

  const handleAdd = () => {
    onAddToCart(item.id, selectedSizeId, selectedSpiceIds)
    handleClose()
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '12px 0 4px' }}>
          <div style={{ width: 40, height: 4, borderRadius: 99, backgroundColor: '#e0e0e0' }} />
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '20px 20px 8px' }}>
          <h2 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 4px', lineHeight: 1.2 }}>
            {item.name}
          </h2>
          {item.description && (
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: active ? 'var(--brand, #d62300)' : '#fff',
                        cursor: 'pointer',
          ...
          {spices.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {spices.map((spice) => {
                  const selected = selectedSpiceIds.includes(spice.id)
                  return (
                    <button
                      key={spice.id}
                      ...
                      style={{
                        ...
                        backgroundColor: selected ? 'rgba(214,35,0,0.06)' : '#fff',
                        cursor: 'pointer',
                        textAlign: 'left',
                        transition: 'all 0.15s ease',
                      }}
                    >
                      ...
                      <span style={{
                        ...
                        backgroundColor: selected ? 'var(--brand, #d62300)' : '#fff',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        flexShrink: 0,
                      ...
                      {Number(spice.price) > 0 && (
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#888', flexShrink: 0 }}>
...
export default function MenuItemCard({
  ...
  const selectedSize = sizes.find((s) => s.id === selectedSizeId) ?? sizes[0] ?? null
  const displayPrice = selectedSize ? Number(selectedSize.price) : Number(item.price ?? 0)

  const currentCartRef = hasSizes
    ? (selectedSizeId ? cartBySizeId[selectedSizeId] ?? null : null)
    : (cartByItemId[item.id] ?? null)
      ...
      <article className="bg-white rounded-2xl overflow-hidden shadow-sm border border-[var(--border)] flex flex-col h-full">
        ...
        >
          {item.images?.[0]?.imageUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-4xl text-[var(--fg-subtle)]">🍗</div>
          )}
          {item.isNew && !item.isFeatured && (
            <span className="absolute top-2 left-2 badge badge-success text-[10px]">Новинка</span>
          )}
          {item.isFeatured && (
            <span className="absolute top-2 left-2 badge badge-brand text-[10px]">Хит</span>
          )}
          {/* Кнопка избранного */}
          <button
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          {item.description && (
            <p className="text-[11px] text-[var(--fg-subtle)] line-clamp-2 mb-2">{item.description}</p>
          )}
          ...
          {hasMultipleSizes && (
            <div className="flex flex-wrap gap-1 mb-2">
              {sizes.map((size) => (
                <button
                  key={size.id}
                  onClick={(e) => { e.stopPropagation(); setSelectedSizeId(size.id) }}
