...
Smart Hero Banner - это интеллектуальный баннер на главной странице пользователя (`/home`), который автоматически показывает наиболее актуальные товары на основе реальных данных из базы.

### Особенности

- ✅ **Автоматическая генерация** - выбирает товары на основе статистики заказов
- ✅ **Живые данные** - использует изображения напрямую из базы данных
- ✅ **Умный выбор** - приоритет популярным товарам, акциям и новинкам
- ✅ **Современный дизайн** - Premium Food Delivery UI стиль
- ✅ **Автоматическая ротация** - смена баннеров каждые 5 секунд
- ✅ **Управление через админку** - полный контроль настроек
  ...
  "banners": [
    {
      ...
      "type": "menu_item",
      "name": "Чикен Бургер",
      "description": "Сочный бургер с хрустящей курочкой",
      "image": "https://...",
      "price": 250,
      "oldPrice": 300,
      "discount": 17,
...
1. **Популярные** - товары с наибольшим количеством заказов за 7 дней
2. **Продаваемые** - товары с высокими продажами за 30 дней
3. **Акции** - комбо и товары со скидкой
4. **Новинки** - товары с флагом `isNew`
5. **Избранные** - товары с флагом `isFeatured`
6. **Случайные** - если статистики нет

### Настройки через админку

Перейдите в `/admin/smart-banner` для управления:

- **Включить/выключить** баннер
- **Время автопрокрутки** (3-10 секунд)
- **Количество баннеров** (1-10)
- **Минимум заказов** для попадания в баннер
- **Фильтры**:
  - Только популярные
  - Только акции
  - Только комбо
...
Использует таблицы:
- `PopularItemsStats` - статистика заказов
- `MenuItem` - товары и их свойства
- `MenuItemImage` - изображения товаров
- `MenuItemSize` - размеры и цены
- `ComboOffer` - комбо-предложения

### Дизайн

- **Высота**: 200px
- **Скругления**: 24px
- **Стиль**: Glassmorphism + Premium Food Delivery
