...
## 🔔 Звуковые уведомления для филиалов

### Функционал

Филиалы получают **циклические звуковые уведомления** о новых оплаченных заказах:

✅ **Автоматический запуск** - звук начинает воспроизводиться при поступлении нового заказа со статусом "pending"
✅ **Бесконечное зацикливание** - звук повторяется пока сотрудник не примет заказ
✅ **Автоматическая остановка** - звук прекращается при изменении статуса заказа (принятие в работу)
✅ **Управление звуком** - кнопка включения/выключения звуковых уведомлений в хедере филиала
✅ **Визуальный индикатор** - показывает количество новых заказов и состояние воспроизведения
✅ **Обход блокировки браузера** - активация звука требует User Gesture (клик по кнопке)

### Как это работает

1. **Включение уведомлений**: Сотрудник филиала нажимает кнопку 🔊 в хедере панели
2. **Поступление заказа**: При новом заказе со статусом "pending" автоматически запускается звук `/uvedomlenie.mp3`
3. **Зацикливание**: Звук повторяется бесконечно (`audio.loop = true`)
4. **Принятие заказа**: Когда сотрудник нажимает "Принять в работу", звук автоматически останавливается
5. **Проверка новых заказов**: Система каждые 10 секунд проверяет наличие новых pending заказов

### Технические детали

**Компонент:** `components/branch/OrderSoundNotification.tsx`

**Интеграция:** Автоматически работает в `app/branch/layout.tsx` для всех филиалов

**Аудиофайл:** `/uvedomlenie.mp3` (48.7 KB)

**API Endpoint:** `GET /api/branch/orders?status=pending`

**Интервал проверки:** 10 секунд

### Преимущества

✅ **Не требует настройки** - работает автоматически для всех филиалов
✅ **Не пропустите заказ** - циклическое воспроизведение пока не примете
✅ **Простое управление** - одна кнопка для включения/выключения
✅ **Совместимость с браузерами** - учитывает политику автовоспроизведения
✅ **Минимальная нагрузка** - эффективный polling без перегрузки сервера
