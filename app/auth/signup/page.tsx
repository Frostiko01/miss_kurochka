...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(
        `/auth/verify-email?email=${encodeURIComponent(formData.email)}&password=${encodeURIComponent(formData.password)}&fullName=${encodeURIComponent(formData.fullName)}`
      );
    } catch (err) {
          ...
          <div className="bg-gradient-to-r from-[#d62300] to-[#ff0000] p-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-2 text-white/90 font-semibold">
                Создайте аккаунт Miss Kurochka
              </p>
            </div>
          </div>
