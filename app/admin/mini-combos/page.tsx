...
export default function AdminMiniCombosPage() {
  ...
  const fetchCombos = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/combo-offers?${params}`);
  ...
  const fetchMenuItems = async () => {
    try {
      const res = await fetch("/api/admin/menu-items?limit=200");
      const data = await res.json();
  ...
  const openEdit = (c: MiniCombo) => {
    ...
    setForm({
      ...
      menuItemIds: c.comboItems.map((ci) => ci.menuItemId),
  ...
  const handleSave = async () => {
    ...
    const data = await res.json();
    if (res.ok) {
      setToast({ message: editing ? "Мини-комбо обновлено" : "Мини-комбо создано", type: "success" });
      setShowModal(false);
      fetchCombos();
    } else {
  ...
  return (
    <div className="p-8 min-h-screen" style={{ backgroundColor: T.bg }}>
      ...
      <div className="mb-8">
        ...
        <p className="font-semibold mt-1" style={{ color: T.muted }}>
          Небольшие наборы, которые отображаются внизу пользовательской страницы
        </p>
      </div>

      {/* Filters */}
      <div className="rounded-2xl p-6 mb-6" style={{ backgroundColor: T.card }}>
      ...
      {/* Content */}
      <div className="rounded-2xl p-6" style={{ backgroundColor: T.card }}>
        {loading ? (
          ...
          <div className="text-center py-12">
            <Zap className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: T.muted }} />
            <p className="font-semibold" style={{ color: T.muted }}>Мини-комбо не найдены</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {combos.map((c) => (
              <div key={c.id} className="rounded-2xl overflow-hidden border transition-all hover:border-[#4047ee]"
                style={{ backgroundColor: T.bg, borderColor: T.border }}>
                <div className="relative h-44">
                  <img src={c.imageUrl} alt={c.name} className="w-full h-full object-cover" />
                  {c.oldPrice && (
                    <span className="absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-extrabold text-white bg-red-500">
                ...
                <div className="p-5">
                    ...
                    <ul className="space-y-0.5">
                      {c.items.map((it, i) => (
                        <li key={i} className="text-sm text-white flex items-center gap-1.5">
                          <span className="w-1 h-1 rounded-full shrink-0" style={{ backgroundColor: T.accent }} />
                          {it}
                        </li>
                      ))}
                    </ul>
                  ...
                  <span className="px-2 py-0.5 rounded-full text-xs font-bold"
                    style={{ backgroundColor: c.isActive ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)", color: c.isActive ? "#22c55e" : "#ef4444" }}>
                    {c.isActive ? "Активно" : "Неактивно"}
                  </span>
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => openEdit(c)} className="flex-1 py-2 rounded-xl font-bold text-white text-sm" style={{ backgroundColor: T.accent }}>
                      Редактировать
                    </button>
                    <button onClick={() => setDeleteModal({ id: c.id, name: c.name })} className="px-3 py-2 rounded-xl font-bold text-white" style={{ backgroundColor: "#ef4444" }}>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ backgroundColor: "rgba(5,12,38,0.82)", backdropFilter: "blur(4px)" }}>
          <div className="rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" style={{ backgroundColor: T.card }}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowModal(false)} className="p-2 rounded-lg" style={{ color: T.muted }}>
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-4">
              <Field label="Название *">
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  ...
                  style={{ backgroundColor: T.bg, borderColor: T.border }} placeholder="Например: Мини-сет" />
              </Field>
              <Field label="Описание">
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  rows={2} className="w-full px-4 py-3 rounded-xl text-white focus:outline-none border"
                  style={{ backgroundColor: T.bg, borderColor: T.border }} />
              </Field>
              <Field label="Блюда в составе *">
                <div className="max-h-48 overflow-y-auto rounded-xl border p-2 space-y-1" style={{ borderColor: T.border, backgroundColor: T.bg }}>
                  {menuItems.map((m) => (
                    <label key={m.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer hover:bg-white/5">
                      <input type="checkbox" checked={form.menuItemIds.includes(m.id)} onChange={() => toggleMenuItem(m.id)}
                ...
                {form.menuItemIds.length > 0 && (
                  <p className="text-xs mt-1" style={{ color: T.muted }}>Выбрано: {form.menuItemIds.length}</p>
                )}
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Цена (сом) *">
                  <input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl text-white focus:outline-none border"
                    style={{ backgroundColor: T.bg, borderColor: T.border }} placeholder="199" />
                </Field>
                <Field label="Старая цена (сом)">
                  <input type="number" value={form.oldPrice} onChange={(e) => setForm({ ...form, oldPrice: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl text-white focus:outline-none border"
                    style={{ backgroundColor: T.bg, borderColor: T.border }} placeholder="299" />
                </Field>
              </div>
              ...
              <Field label="Изображение *">
                <ImageUpload value={form.imageUrl} onChange={(url) => setForm({ ...form, imageUrl: url })} folder="combos" />
              </Field>
              <label className="flex items-center gap-3">
                ...
                <span className="text-white font-semibold">Активно</span>
              </label>
              <div className="flex gap-3 pt-2">
                <button onClick={handleSave} className="flex-1 py-3 rounded-xl font-bold text-white" style={{ backgroundColor: T.accent }}>
                  {editing ? "Сохранить" : "Создать"}
                </button>
                <button onClick={() => setShowModal(false)} className="px-6 py-3 rounded-xl font-bold text-white" style={{ backgroundColor: T.cardAlt }}>
                  Отмена
                </button>
              </div>
            </div>
